...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(result)
        ops = rest.map(lambda operands: scheme_eval(operands, env))
        return scheme_apply(result, ops, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('youre passing in wrong arg numbers')
        while len(formals) > 0:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        mapped_ops = operands.map(lambda op: scheme_eval(op, env))
        return scheme_apply(self, mapped_ops, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lam_proc = do_lambda_form(Pair(target.second, expressions.second), env)
        env.define(target.first, lam_proc)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def values_and_formals(binding):
        if binding is nil:
            return binding, binding
        else:
            check_form(binding.first, 2, 2)
            form = Pair(binding.first.first, values_and_formals(binding.second)[0])
            val = Pair(scheme_eval(binding.first.second.first, env),
                values_and_formals(binding.second)[1])
            return form, val

    formals, values = values_and_formals(bindings)
    check_formals(formals)
